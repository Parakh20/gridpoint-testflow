...
// SecureStore key rules: alphanumeric, ".", "-", "_" only — no colons.
const SEP = '__';

async function removeChunks(key: string, knownCount?: number) {
  const countStr = await SecureStore.getItemAsync(`${key}${SEP}count`);
  const count = knownCount ?? (countStr ? parseInt(countStr, 10) : 0);
  await SecureStore.deleteItemAsync(`${key}${SEP}count`);
  for (let i = 0; i < (Number.isFinite(count) ? count : 0); i++) {
    await SecureStore.deleteItemAsync(`${key}${SEP}${i}`);
  }
}
...
async function setItem(key: string, value: string) {
  ...
  for (let i = 0; i < chunks; i++) {
    ...
    await SecureStore.setItemAsync(`${key}${SEP}${i}`, slice);
  }
  ...
  await SecureStore.setItemAsync(`${key}${SEP}count`, String(chunks));
}
...
async function getItem(key: string): Promise<string | null> {
  ...
  for (let i = 0; i < count; i++) {
    const part = await SecureStore.getItemAsync(`${key}${SEP}${i}`);
    if (part === null) {
