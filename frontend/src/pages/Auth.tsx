...
import { useAuth, type OAuthProviderKey } from '@/contexts/AuthContext';
...
export default function Auth() {
  ...
  const { signIn, signInWithOAuth, resetPasswordForEmail, updatePassword, companyMismatch, accountDisabled, oauthPending } = useAuth();
  ...
  const handleOAuthSignIn = async (provider: OAuthProviderKey) => {
    if (company?.oauth_provisioning === 'off') {
      toast({ variant: 'destructive', title: `${PROVIDER_LABEL[provider]} sign-in not enabled`, description: 'Contact your administrator to enable third-party sign-in for this workspace.' });
      return;
    }
    ...
    if (error) {
      ...
      toast({ variant: 'destructive', title: `${PROVIDER_LABEL[provider]} sign-in failed`, description: error.message });
    }
            ...
            <div className="rounded-2xl border border-border bg-card/80 p-8 shadow-2xl backdrop-blur-xl">
              {oauthPending ? (
                <div className="flex flex-col items-center gap-4 py-4 text-center">
                  ...
                  <div>
                    <h3 className="text-lg font-semibold text-foreground">Awaiting approval</h3>
                    <p className="mt-2 text-sm text-muted-foreground">
                      Your Google sign-in was received. Your workspace administrator
                      <br />
                ...
                <>
                  ...
                  <div className="mt-3 space-y-2">
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2.5 h-11 rounded-lg border border-border bg-card hover:bg-accent transition text-sm font-medium text-foreground disabled:opacity-60"
                    >
                      {oauthLoading === 'google' ? (
                    ...
                    {/* Microsoft/LinkedIn/Apple: AuthContext.signInWithOAuth already
                        supports these (see OAUTH_PROVIDERS), buttons deliberately
                        hidden until each provider is registered + enabled in
                        Supabase Dashboard — planned once paid pilots justify the
                        setup. Re-add the three buttons from git history when ready. */}
                  </div>
                </>
              )}
