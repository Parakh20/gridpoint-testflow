...
- **Billing model: PREPAID PERIODS, no autopay (2026-08-27).** A company buys a month or a year outright as a **one-time Razorpay order**. There is no e-mandate and nothing is ever charged automatically. `subscriptions.current_period_end` is the only thing that matters: past it the workspace is **read-only** until someone renews. Chosen because Indian e-mandate is high-friction — bank support is uneven, corporate cards often cannot mandate at all, procurement expects to pay an invoice, and the live cutover probe hit exactly this (Google Pay refused to configure autopay; a plain debit card worked first time).
- **Renewal flow:** `manage-subscription`'s `renew_plan` action (SUPERADMIN-checked server-side) prices the period from `plans` — never from the request body — and creates a Razorpay order with `notes.order_type='plan_renewal'`, `plan_slug`, `billing_interval`. `RenewPlanCard.tsx` opens Checkout.js against that `order_id`. On `payment.captured`, `razorpay-webhook` writes `orders` **and** calls `apply_plan_period`, which is idempotent on `subscriptions.last_renewal_payment_id` — a redelivered webhook cannot hand out a free period. Early renewal extends from `current_period_end`, not from today, so nobody loses days by paying early; a lapsed company starts fresh from today. **Renewing into a different plan is the same action as renewing into the same one** — with no continuous subscription there is no upgrade/downgrade path to maintain, so `subscribe`/`upgrade`/`downgrade`/`cancel` were all removed along with `SubscribeCard`/`SubscriptionActions`.
- **Freeze (read-only):** `is_workspace_frozen(company)` is the single definition — `NOW() > current_period_end`, falling back to `companies.trial_ends_at` when no period was ever bought (a NULL `trial_ends_at` is a grandfathered company and is never frozen). Enforced in `can_create_project()` (which the `projects` INSERT policy already calls) and by **RESTRICTIVE** RLS policies on `test_records`/`nameplate_records` INSERT+UPDATE. `AS RESTRICTIVE` is essential: permissive policies are combined with OR, so a plain `FOR INSERT` policy would have granted a new way in rather than closing one. SELECT is deliberately untouched — a frozen customer can still see and export everything, which is both the humane behaviour for a field team mid-commissioning and the strongest renewal argument there is.
- **Renewal reminders:** `queue_renewal_reminders()` queues into the `renewal_notifications` outbox at 7/3/1 days before `current_period_end` plus one on lapse; `notify-renewal` Edge Function drains it and emails every active SUPERADMIN via Resend. Daily cron at 04:00 UTC (09:30 IST) via `.github/workflows/notify-renewal.yml`, `X-Cron-Secret`-gated. `UNIQUE(company_id, period_end, days_before)` makes a double-fired cron a no-op. With no autopay, this is the mechanism the whole model depends on — an unreminded customer simply lapses.
- **Razorpay subscriptions are dead.** `subscription.*` webhook events are acknowledged and ignored. `reconcile-cancellations` (function + workflow) is deleted. The six live Razorpay plans and `plan_provider_mapping` are now only historical — renewal orders carry their own amount, so plan prices are editable again in `plans` without the create-a-new-plan-and-remap dance (gotcha 25 applies only to the dead subscription path).
