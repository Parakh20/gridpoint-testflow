...
| `MIGRATIONS.md` | Migration playbook — naming, idempotency, rollback strategy, RLS testing |
...
| `health` | Public uptime probe. Returns `{ ok, db, version, ts }`. 200 on success, 503 if DB unreachable. No auth required. |
| `razorpay-webhook` | **Scaffold.** Verifies HMAC-SHA256 signature against `RAZORPAY_WEBHOOK_SECRET`, upserts `subscriptions` row via `upsert_subscription` RPC. Activate by creating Razorpay account, configuring webhook, setting the secret. Maps `subscription.notes.company_id` → `subscriptions.company_id`. |
...
24. **Soft delete on `projects`, `equipment_instances`, `test_records`** — these tables have a `deleted_at TIMESTAMPTZ` column. The SELECT RLS policy filters them out by default. **SUPERADMIN sees soft-deleted rows** so they can restore via the `restore_project(_project_id)` RPC. Do not write code that does `.delete()` on these tables in normal flows — set `deleted_at = NOW()` via UPDATE instead. The existing `DELETE` policies still exist for hard-delete by SUPERADMIN if truly needed.
25. **Audit logs are written by Postgres triggers, not the app.** `log_audit_event()` trigger fires on INSERT/UPDATE/DELETE for `projects`, `equipment_instances`, `test_tasks`, `test_records`, `user_roles`. Triggers run in the same transaction as the mutation — they can't be skipped on app crash. **Do not** keep adding app-level audit inserts for these tables; the trigger covers them. App-level inserts are still appropriate for non-table events (e.g. login, password reset).
26. **`scope_items.quantity` is bounded 1–500.** A `CHECK` constraint enforces this at the DB level. The frontend should also validate but the DB is the source of truth — a malformed API call will get a `23514` error.
27. **Idle timeout: 30 minutes.** `AuthContext` registers global `mousedown`/`keydown`/`touchstart`/`scroll`/`pointermove` listeners and signs out after 30 min idle. Configurable via `IDLE_TIMEOUT_MS` constant.
28. **CSP allow-list.** `vercel.json` ships a strict Content-Security-Policy. When integrating a new third-party (Razorpay/Sentry/etc.), update the `connect-src` and possibly `frame-src` / `script-src` entries. Test in production preview before deploying.
29. **Database backups: `.github/workflows/backup.yml`.** Nightly `pg_dump` at 03:00 UTC. Inactive until you set `SUPABASE_DB_URL` + S3 secrets. When S3 creds are missing, it still keeps a 14-day Actions artifact as fallback. Trigger manually via `workflow_dispatch`.
30. **Bundle is split.** `xlsx` (Excel export) and `jspdf`/`html2canvas` (PDF export) are dynamically imported at call sites — they only land in the user's browser when an export is triggered. `recharts`, `radix-ui`, and `@supabase/supabase-js` are split into separate cacheable chunks via `vite.config.ts` `manualChunks`. Don't add static imports of these heavy libs in app code.
31. **Project clone:** `clone_project(_source_project_id, _new_number, _new_site_name, _new_address)` RPC. Copies scope_items + project_test_scope only; equipment instances and test records are intentionally not cloned. New project is created in DRAFT. UI lives in `ProjectStatusActions` ("Clone" button, visible to GM/SUPERADMIN in any status).
32. **Audit log viewer UI** lives at `AuditLogViewer.tsx`, embedded as a card in `SuperadminDashboard`. Lazy-loads expanded before/after JSON on row click. Reads the last 500 entries; filter by table/action/free-text search.
33. **Bulk approve test tasks:** SupervisorDashboard adds checkboxes to each pending task. The bulk UPDATE includes `.eq('status', 'SUBMITTED')` as a concurrency guard so rows already moved by someone else aren't clobbered.
34. **Edge Functions are rate-limited** via `rate_limit_check(_key, _limit, _window_minutes)` RPC backed by the `rate_limits` table. Wired into `create-user` (30/hr per SUPERADMIN), `delete-user` (30/hr), `generate-report` (10/hr), `create-tenant` (20/hr per IP). Helper lives in `supabase/functions/_shared/rate_limit.ts` — call `enforceRateLimit(supabaseAdmin, req, { key, limit, windowMinutes }, cors)` at the top of any new function that mutates auth or external APIs. Fails open if the RPC itself errors.
35. **Trial mode + feature flags** live on `companies.trial_ends_at` (defaulted to NOW()+14d by `set_default_trial` BEFORE-INSERT trigger) and `companies.features` (JSONB, default `{}`). Use `has_feature('flag')` SQL function for RLS-level gates, `useFeature('flag')` hook in React. Flags default to TRUE when unset. `TrialBanner` in `DashboardLayout` shows days-remaining countdown and urgent past_due/expired banners.
36. **Razorpay billing scaffolded.** `subscriptions` table holds one row per company keyed by `company_id`. `upsert_subscription` RPC is the only mutation path — called from `razorpay-webhook` Edge Function after signature verification. No actual Razorpay account, plan, or checkout flow is wired yet. To activate: create Razorpay account, plans, set `RAZORPAY_WEBHOOK_SECRET` via `supabase secrets set`, build the frontend checkout flow.
37. **Password policy:** 10 chars minimum + uppercase + lowercase + digit, enforced both client-side (zod in `InviteUserDialog`) and server-side (Edge Functions `create-user`, `create-tenant`). `generatePassword()` in both invite dialogs guarantees compliance via seed-then-shuffle.
38. **User offboarding** via `offboard_user(_from_user, _to_user)` RPC. Transfers all `projects.created_by`, `projects.assigned_to`, `equipment_instances.assigned_to`, `test_tasks.assigned_to` in one transaction, then sets `profiles.is_active = false`. UI: `OffboardUserDialog` in UserManagementTable shows `user_workload_summary` and prompts for transferee. Do **not** delete users with open work — always offboard first.
39. **Error monitoring** abstraction in `lib/monitoring.ts`. Currently a console-only no-op. To activate Sentry: `npm install @sentry/react`, set `VITE_SENTRY_DSN`, uncomment the `SENTRY:` markers in the file. `ErrorBoundary` already calls `captureException` so wiring once activates global capture.
40. **E2E tests** scaffolded in `frontend/e2e/` (Playwright). One golden-path test plus a health endpoint probe. Activate: `npm install -D @playwright/test`, `npx playwright install --with-deps chromium`, set `E2E_SUPERADMIN_EMAIL`/`E2E_SUPERADMIN_PASSWORD`, run `npx playwright test`.
41. **Magic link `redirectTo` is ignored by Supabase unless the URL is manually rewritten after generation.** `admin.generateLink({ options: { redirectTo } })` has no effect — Supabase always uses the configured Site URL. Workaround: after calling `generateLink`, take `linkData.properties.action_link`, parse it as a URL, call `url.searchParams.set('redirect_to', targetUrl)`, and use the rewritten URL. See `get_company_magic_link` in `supabase/functions/platform-admin-data/index.ts`.
